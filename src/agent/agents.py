...
class AgentFactory:
    ...
    def __init__(self):
        self._search_and_summarise_agent = None
        self._github_assistant = None
    ...
    async def get_search_and_summarise_agent(self) -> CompiledStateGraph:
        if self._search_and_summarise_agent is None:
            ...
            self._search_and_summarise_agent = create_react_agent(
                model=agent_cfg.model,
                tools=[is_valid_link, search_tool],
                debug=agent_cfg.debug,
                prompt=summarised_prompt,
                name=agent_cfg.name,
                response_format=SummariseAgentResponse,  # Could be made dynamic if needed
            )
        return self._search_and_summarise_agent

    async def get_github_assistant(self) -> CompiledStateGraph:
        if self._github_assistant is None:
            ...
            self._github_assistant = create_react_agent(
                model=agent_cfg.model,
                tools=tools,
                debug=agent_cfg.debug,
